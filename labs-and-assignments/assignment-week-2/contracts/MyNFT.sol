...
contract nftMint is ERC721, Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    constructor(string memory baseURI) ERC721("SKYTOKEN","SKY") {
        setBaseURI(baseURI);
    }

    function mintNFTs(uint256 _count) public payable {
        uint totalMinted = _tokenIds.current();
        require(totalMinted.add(_count) <= maxSupply, "Not enough NFTs remaining to mint.");
        require(_count > 0 && _count <= maxPerMint, "Cannot mint specified number of NFTs.");
        require(msg.value >= cost.mul(_count), "Not enough Ether in wallet to purchase NFTs.");
